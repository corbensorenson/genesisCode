...
The bundle is byte-reproducible gzip/USTAR with fixed metadata and a closed artifact manifest. `submit` performs no network operation and writes a content-addressed immutable outbox envelope. This transport-neutral execution evidence is not yet a ranked result: registry admission must independently validate and rescore it and bind signed submitter, track, contamination, cohort, and hardware evidence.
