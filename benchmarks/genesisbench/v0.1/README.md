...
Before local execution, use `--verify-local` with one explicit `ID=SNAPSHOT_ROOT` binding for every candidate. This proves the machine still has the exact preselected bytes. Open Agent v0.5 now supplies the auth-free `mlx-responses` provider, zero transport retries, exact Responses-to-MLX adapter/runtime custody, separate outer read/write/network sandboxes, a closed observed tool scaffold, retained wire/rejection evidence, and hard adapter/model-server teardown. `local-models/custody/qwen3-4b-4bit-v0.1.json` is the first complete custody manifest. It is still not a score: the 4B reality gate must be predeclared and run exactly once, validated and replayed without weights, then published before the separately predeclared 8B cohort begins.
