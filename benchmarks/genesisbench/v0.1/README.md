...
Before local execution, use `--verify-local` with one explicit `ID=SNAPSHOT_ROOT` binding for every candidate. This proves the machine still has the exact preselected bytes. Open Agent v0.5 now supplies the auth-free `mlx-responses` provider, zero transport retries, exact Responses-to-MLX adapter/runtime custody, separate outer read/write/network sandboxes, a closed observed tool scaffold, retained wire/rejection evidence, and hard adapter/model-server teardown. The 4B and 8B custody manifests bind the independently selected `smallest-adapter-viable` and `strongest-host-fit` classes, respectively. Neither manifest is a score: the frozen 4B calibration reality gate must be predeclared and run exactly once, validated, replayed, and published before the separately predeclared 8B capability cohort begins. Results remain separate by model identity and capacity tier; the 4B cohort cannot support strongest-host-fit or headline capability claims.
