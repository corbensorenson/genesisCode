...
- [ ] Decompose Rust megafiles into domain modules with stable internal interfaces.
  ...
  - Remaining for closure: split `gc_effects::runner_capability_dispatch` capability arms by domain (`store/refs/sync/pkg/vcs/gc`) into focused command modules; split `gc_opt::stage2_wasm` planning/lowering/execution pipelines into staged modules.
