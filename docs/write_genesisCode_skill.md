...
- Keep evaluation roles disjoint: public references support development only. Never retrieve private held-out packs for authoring or training; bind evaluator results to the active 90-lineage public commitment snapshot and its hash-only audit, and default contamination to `unknown` when release chronology or model training provenance is incomplete.
...
- Use the closed benchmark-run record for provenance: bind immutable model/runtime artifacts, exact prompt/card/context assembly, integer decoding/retry controls, every attempt and candidate artifact, canonical score, normalized host facts, and the complete inventory. Never accept a stale or partial record.
- Run local benchmark models only through the digest-pinned `genesis.agent-model-runner.v0.1` / `infer` effect. Keep request, response, transcript, and `.gclog`; replay must work after removing the model and weights.
