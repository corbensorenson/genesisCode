...
Production DSSE admission is decided by the artifact-loaded `core/security::evidence-verify-authority` under `SELFHOST_EVIDENCE_VERIFY_AUTHORITY_v0.1.md`. The host may decode bounded bytes and execute SHA-256 and Ed25519, but it cannot return a successful verification without the request-bound GenesisCode verdict. The standalone evidence verifier remains independent corroboration and is never a production fallback.
