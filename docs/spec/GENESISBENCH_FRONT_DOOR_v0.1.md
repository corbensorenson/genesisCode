...
The Open Agent extension is a separate content-addressed execution boundary. It does not add a sixth class to `GENESISBENCH_ADAPTERS_v0.1.json`, change Cold Acquisition, or make custom orchestration comparable to the fixed reference scaffold. New campaigns use `GENESISBENCH_OPEN_AGENT_v0.5.json`; immutable v0.1-v0.4 authorities remain available for historical replay. v0.5 preserves v0.4's complete transitive implementation binding and byte-complete invalid-workspace retention, then adds credential-free local MLX execution under separate outer Codex and provider sandboxes, a closed model/license/Python/distribution/adapter custody manifest, an exact Responses-to-MLX translation, retained request/response/event/rejection evidence, zero transport retries, no server fallback, and hard model-server termination and reaping. Its recursively closed campaign, attempt-predeclaration, run, campaign-report, replay-tool archive, and MLX-custody contracts are `GENESISBENCH_OPEN_AGENT_CAMPAIGN_v0.1.schema.json`, `GENESISBENCH_OPEN_AGENT_PREDECLARATION_v0.1.schema.json`, `GENESISBENCH_OPEN_AGENT_RUN_v0.1.schema.json`, `GENESISBENCH_OPEN_AGENT_CAMPAIGN_REPORT_v0.1.schema.json`, `GENESISBENCH_OPEN_AGENT_TOOL_ARCHIVE_v0.1.schema.json`, and `GENESISBENCH_MLX_CUSTODY_v0.1.schema.json`.

`agent-campaign-plan` must run before model inference. It atomically binds the complete 9-case reality gate or 27-case public matrix, all protocol/suite/snapshot/scaffold identities, exact Codex executable digest and version, supplied Genesis executable and self-host artifact digests, requested model and revision, reasoning effort, local model artifact and custody identities when applicable, hardware class, secret handling, one-attempt policy, capabilities, environment-name allowlist, finite wall-time/output/workspace budgets, non-selective stop rules, and expected-attempt count. `agent-plan` may only derive an attempt for a case already named by that immutable campaign; the attempt binds the campaign identity and repeats the common fields exactly. Neither contract records environment or secret values. Execution and replay resolve the scoring authority and task suite from that frozen protocol/suite binding, including from the pinned Git snapshot after current authorities advance. Every harness predeclaration is deterministically `rankEligible: false`; only the independent signed registry may derive ranked admission after verifying provenance, contamination, artifact, cohort, replay, and rescore evidence. Passing `--immutable-revision` is a provenance assertion and is appropriate only when the provider exposes a genuinely immutable revision. A v0.5 local cohort requires `--local-provider mlx-responses`, `--model-artifact-sha256`, and the exact `--local-custody-manifest`; execution additionally receives the locally verified model root and Python executable, but neither host path enters canonical evidence.

A local run is never scored merely because Codex exits successfully. The validator independently derives `provider-no-request` when no backend request occurred and `provider-policy-rejection` when the adapter rejected any request or observed a forbidden credential or hidden retry. Those facts remain in the replayable bundle, force an `invalid` outcome, and prohibit `score.json`. The provider's complete process group must be reaped before evidence is admitted.

The v0.5 local cohort accepts only `reasoningEffort: low` and records the exact translation `codex-low-to-qwen-thinking-disabled-v0.1`. The adapter requires Codex's closed `{"effort":"low","summary":"auto"}` Responses field, while MLX starts Qwen with `enable_thinking=false`. Local `xhigh`, `high`, or `medium` claims are rejected rather than silently dropped. These local results are not reasoning-effort comparable to hosted Luna `xhigh`.

`agent-run` reconstructs the frozen protocol snapshot from the pinned Git commit and verifies every archived byte before execution. The process workspace is outside the repository ancestry so project `AGENTS.md`, skills, configuration, and other ambient instructions cannot be discovered through parent traversal. The snapshot and copied GenesisCode toolchain live under a read-only input root outside the writable case workspace. The fixed Codex invocation is ephemeral, JSONL, non-resumable, `workspace-write`, approval-free, and one prompt; hosted and loopback-local inference are separate invocation profiles. The process receives only the closed environment-name allowlist. v0.3 disables the supplied tool's compiled self-host cache, admits and retains newly created declared editable outputs, permits a bounded event line up to the total capture ceiling, and replaces the ephemeral staging prefix in retained events and stderr with `$GENESISBENCH_STAGE`. Timeout or capture overflow kills and reaps the complete process group. Permission-aware cleanup removes every protected temporary snapshot after its retained evidence is atomically published.
...
Independent rescore is toolchain-specific. When a campaign retains `tools/archive.json`, replay validates that manifest against the immutable campaign identity and authorities, verifies the compressed and uncompressed executable plus self-host artifact, and scores with those archived bytes instead of the current installation. This rule also closes the single verified v0.2 Luna attempt after the runtime advances; a current executable is never accepted as equivalent merely because it reports the same version.
...
The authority test executes identical vectors through all five fixed adapters, then runs Open Agent controls for hidden retries, capability broadening, model/tool-claim forgery, implementation substitution or omission, declared-output and invalid-workspace retention, cache suppression, historical scoring-authority resolution, snapshot byte/mode/symlink/topology mutation, bounded event lines, ephemeral-path normalization, malformed events, and descendant survival after timeout. The integration test exercises both public surfaces, deletes external adapter/agent executables before replay, independently rescores, proves byte-identical bundles, submits to an immutable outbox, retains failures, and rejects candidate or observed-workspace tampering.
