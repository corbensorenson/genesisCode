...
the tree-walk module frame and compiled module cells can form direct or indirect ownership cycles.
The Rust tiers place every cycle-capable runtime owner behind trace-aware `Shared<T>` nodes and run
explicit collection only after an outer evaluation/application boundary unwinds. Stable hash cycle
breaking remains distinct from reclamation. No new execution tier may omit a graph edge, place a host
handle inside a kernel value, expose reclamation timing, or substitute collector bytes or process RSS
for deterministic logical allocation and live-heap charges.
