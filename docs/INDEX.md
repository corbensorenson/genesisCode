...
- `docs/spec/GENESISBENCH_OPEN_AGENT_v0.1.json`, its predeclaration/run schemas, and `scripts/lib/genesisbench_open_agent.py` - separate capability-minimal repository-editing harness for predeclared Codex CLI hosted or local cohorts; frozen read-only source, one writable case workspace, exact executable/model bindings, one attempt, hard process-group termination, closed transcripts/candidates, and replay without agent/model access.
