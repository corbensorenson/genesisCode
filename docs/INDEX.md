...
- `docs/spec/GENESISBENCH_OPEN_AGENT_v0.2.json`, retained v0.1 authority, campaign/predeclaration/run/report schemas, and `scripts/lib/genesisbench_open_agent.py` - separate capability-minimal repository-editing harness for predeclared Codex CLI hosted or local cohorts; atomic complete-matrix commitment, ancestry-isolated staging, deterministic read-only-snapshot teardown, one writable case workspace, exact executable/model bindings, one attempt, hard process-group termination, closed transcripts/candidates, and replay without agent/model access. `scripts/lib/genesisbench_open_agent_report.py` derives the complete campaign disposition without model access.
- `benchmarks/genesisbench/v0.1/authority-archive/` - append-only identity-addressed task-suite and protocol authorities required to validate historical Open Agent campaigns after current generated authorities advance.
