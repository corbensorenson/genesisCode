...
- `docs/spec/GENESISBENCH_OPEN_AGENT_v0.5.json`, retained v0.1-v0.4 authorities, campaign/predeclaration/run/report/tool-archive schemas, and `scripts/lib/genesisbench_open_agent.py` - separate capability-minimal repository-editing harness for predeclared Codex CLI hosted or local cohorts; atomic complete-matrix commitment, ancestry-isolated staging, transitively content-bound implementation, declared output and invalid-workspace retention, exact executable/model/tool/custody bindings, one attempt, hard process-group termination, closed transcripts/candidates/provider wire evidence, and replay without agent/model access. v0.5 adds credential-free MLX execution through independently sandboxed Codex and provider processes, rejects zero-request or policy-rejected runs before scoring, and reaps the model server. `scripts/lib/genesisbench_open_agent_report.py` derives typed campaign dispositions without model access.
- `docs/spec/GENESISBENCH_LOCAL_MODELS_v0.1.schema.json`, `docs/spec/GENESISBENCH_MLX_CUSTODY_v0.1.schema.json`, `benchmarks/genesisbench/v0.1/local-models/`, `scripts/lib/genesisbench_local_models.py`, `genesisbench_mlx_custody.py`, and `genesisbench_mlx_responses.py` - score-blind five-snapshot preselection, exact model inventories, retained upstream license evidence, and the active local custody/transport boundary. The Qwen3 4B manifest binds all model, adapter, Python, and transitive MLX distribution bytes without publishing weights or host paths. File, network, credential, retry, tool, context, model-substitution, and server-survival controls fail closed. This is execution-readiness evidence, not a GenesisBench quality score.
...
- `docs/spec/SEMANTIC_REFACTOR_PLAN_v0.1.md` - closed artifact-only GenesisCode workspace refactor planning protocol, deterministic operation topology, and independent host-verification boundary.
...
- `docs/spec/SELFHOST_STORE_AUTHORITY_v0.1.md` - normative R4.2.e artifact-store and H2 artifact-GC semantic authority, exact-mechanism boundary, resource contracts, and non-closure scope.
...
- `docs/spec/SELFHOST_GC_AUTHORITY_v0.1.schema.json` - closed schema for the independently checked H2 artifact-GC authority profile.
...
- `docs/spec/SELFHOST_REFS_AUTHORITY_v0.1.md` - normative partial R4.2.e direct reference database authority, atomic host mechanism, and residual policy/bulk-update boundary.
- `docs/spec/SELFHOST_REFS_AUTHORITY_v0.1.schema.json` - closed schema for the independently checked partial refs-authority profile.
- `policies/selfhost_refs_authority_v0.1.json` - content-addressed direct reference decision inventory, bounded host mechanisms, and explicit nonclaims.
- `scripts/lib/selfhost_refs_authority.py` - independent refs source/manifest/production-route/atomic-adapter custody and mutation verifier.
- `docs/spec/SELFHOST_PKG_LOCK_WRITE_AUTHORITY_v0.1.md` - normative partial R4.2.e package-lock normalization, canonical-byte, identity, and atomic persistence boundary.
- `docs/spec/SELFHOST_PKG_LOCK_WRITE_AUTHORITY_v0.1.schema.json` - closed schema for the independently checked partial package-lock-write authority profile.
- `policies/selfhost_pkg_lock_write_authority_v0.1.json` - content-addressed package-lock decision inventory, bounded host mechanisms, and explicit nonclaims.
- `scripts/lib/selfhost_pkg_lock_write_authority.py` - independent package-lock source/artifact/production-route/ledger custody and mutation verifier.
- `docs/spec/SELFHOST_PKG_LOCK_READ_AUTHORITY_v0.1.md` - normative partial R4.2.e typed package-lock read authority and residual generic TOML host boundary.
...
- `scripts/lib/selfhost_pkg_snapshot_authority.py` - independent package-snapshot source/artifact/route/order/ledger custody and mutation verifier.
...
- `docs/spec/SELFHOST_PKG_VERIFY_AUTHORITY_v0.1.md` - normative partial R4.2.e verify plan, observation-prefix binding, fail-fast disposition, and report authority boundary.
...
- `docs/spec/SELFHOST_PKG_PACKAGE_MANIFEST_AUTHORITY_v0.1.schema.json` - closed schema for the independently checked structural package-manifest authority profile and its explicit effects residual.
...
- `docs/spec/RELEASE_EVIDENCE_AGGREGATE_v0.2.schema.json` - closed read-only aggregate decision schema for ten hosted v0.2 workers, four named-target dispositions, and independently reconciled Linux/macOS host-handle lifecycle custody.
