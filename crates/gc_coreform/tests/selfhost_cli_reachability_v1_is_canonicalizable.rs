...
#[test]
fn selfhost_cli_reachability_v1_is_canonicalizable_and_idempotent() {
    let path: PathBuf = [
        env!("CARGO_MANIFEST_DIR"),
        "..",
        "..",
        "selfhost",
        "cli_reachability_v1.gc",
    ]
    .iter()
    .collect();
    let src = std::fs::read_to_string(&path).expect("read selfhost/cli_reachability_v1.gc");

    let forms = parse_module(&src).expect("parse selfhost/cli_reachability_v1.gc");
