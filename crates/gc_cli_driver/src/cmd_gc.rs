...
pub(super) fn cmd_gc(
    ...
    let kind = gc_contract::kind(cmd);
    let log_op = gc_contract::log_op(cmd);
    let (prog, program_hash) = if frontend_is_rust(&frontend) {
        let forms = match cmd {
            ...
            GcCmd::Pin { target, pins } => mk_gc_pin_program(target, pins),
            GcCmd::Unpin { target, pins } => mk_gc_unpin_program(target, pins),
            GcCmd::Purge {
                ttl_days,
                quarantine_dir,
