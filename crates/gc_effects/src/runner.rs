...
fn validate_locked_entries_strict(
    store: &ArtifactStore,
...
fn workspace_snapshot_term_from_lock(lock: &gc_pkg::GenesisLock) -> Term {
    let modules = lock
        .locked
        .iter()
        .map(|(name, le)| {
...
fn persist_workspace_root_snapshot(
    store: &ArtifactStore,
...
fn locked_dependency_provenance(
    store: &ArtifactStore,
    locked: &BTreeMap<String, gc_pkg::LockedEntry>,
    strict: bool,
    error_tok: SealId,
...
fn commit_provenance_term(commit: &gc_vcs::Commit) -> Term {
    Term::Map(
        [
            (
                TermOrdKey(Term::symbol(":parents")),
