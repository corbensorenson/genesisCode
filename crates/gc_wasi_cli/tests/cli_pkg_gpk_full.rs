...
#[test]
fn wasi_pkg_export_import_gpk_full_includes_commit_patch_evidence_and_embedded_refs() {
...
"#,
    ...
    let bundle_out = cmd()
        ...
        .stdout
        .clone();
    let bundle_h = String::from_utf8(bundle_out).unwrap().trim().to_string();
    assert!(is_hex64(&bundle_h));
