...
#[test]
fn wasi_gc_plan_then_run_deletes_unreachable_artifacts() {
...
#[test]
fn wasi_gc_quarantine_and_purge_roundtrip() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
...
#[test]
fn wasi_gc_unpin_allows_reclaim_after_run() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
    ...
    let pin_log = dir.join("gc-pin3.gclog");
    cmd()
        .current_dir(dir)
        .arg("--json")
        .args(["gc", "--caps"])
    ...
    let unpin_log = dir.join("gc-unpin3.gclog");
    cmd()
        .current_dir(dir)
        .arg("--json")
        .args(["gc", "--caps"])
...
#[test]
fn wasi_gc_pin_ref_keeps_target_even_with_no_refs_root_scan() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
