...
#[test]
fn wasi_gpk_shallow_includes_contract_snapshot_overrides() {
...
#[test]
fn wasi_gpk_shallow_includes_module_snapshot_defs() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
    let caps = write_caps(dir);
...
#[test]
fn wasi_gpk_shallow_includes_workspace_snapshot_modules_transitively() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
    let caps = write_caps(dir);
