...
fn dispatch(cli: &Cli) -> Result<CmdOut, CliError> {
    ...
    match &cli.cmd {
        ...
        Cmd::Run {
            ...
            caps,
            log,
        } => cmd_run(cli, file, *engine, caps, log.as_deref()),
        Cmd::Replay {
            file,
            engine,
            log,
    ...
    match &cli.cmd {
        ...
        other => {
            ...
            Err(cli_err(
                ...
                format!(
                    "selfhost-only mode currently supports only `fmt`, `eval`, `explain`, `run`, `replay`, `optimize`, `typecheck`, `test`, `apply-patch`, `pack`, `store`, `refs`, `pkg`, `policy`, `sync`, `gc`, `selfhost-dashboard`, and `vcs/*`; `{cmd}` is not yet selfhost-routed"
                ),
            ))
        }
    }
