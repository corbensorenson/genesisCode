...
#[test]
fn pin_vs_track_lock_and_update_are_deterministic() {
  ...
  :message "c2"
    ...
    // Policy that allows refs/heads/main with no obligations/signatures.
...
"#,
    ...
    // Init lock + add pinned and tracked requirements.
    cargo_bin_cmd!("genesis")
        .current_dir(dir)
        .args(["pkg", "--caps"])
        .arg(&caps)
    ...
    // Lock once: both should resolve to c1/snap1.
    cargo_bin_cmd!("genesis")
        .current_dir(dir)
        .args(["pkg", "--caps"])
        .arg(&caps)
    ...
    // Advance the ref -> c2 and run update. Only tracked should move.
    cargo_bin_cmd!("genesis")
        .current_dir(dir)
        .args(["refs", "--caps"])
        .arg(&caps)
