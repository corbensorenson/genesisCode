...
#[test]
fn pkg_export_import_gpk_full_includes_commit_patch_evidence_and_embedded_refs() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
    let caps = write_caps(dir);

    // Minimal package -> snapshot.
    fs::write(
...
"#,
    ...
    // Extra artifacts referenced by patch/evidence so full closure must include them.
...
"#,
    ...
    // Set local ref so export can embed it.
    ...
    // Export full bundle rooted at commit, with embedded ref.
    let bundle = dir.join("mini-full.gpk");
    ...
    assert_eq!(
        m.get(&TermOrdKey(Term::symbol(":bundle-v"))),
        Some(&Term::Int(2.into()))
    );
    let Term::Vector(rs) = m
        .get(&TermOrdKey(Term::symbol(":refs")))
        .expect("missing :refs")
        .clone()
    else {
        panic!(":refs must be vector");
    };
    assert_eq!(rs.len(), 1);

    // Verify closure artifacts exist after import (commit + snapshot + patch + evidence + their refs).
    for h in [
        &commit_h,
        &snapshot_h,
        &patch_h,
...
#[test]
fn pkg_export_full_from_ref_with_include_evidence_none_excludes_evidence_artifacts() {
    let td = tempfile::tempdir().unwrap();
    let dir = td.path();
    let caps = write_caps(dir);
