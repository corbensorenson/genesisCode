...
if [[ "$#" -ne 5 ]]; then
  echo "usage: $0 <report-output> <history-output> <report-input> <history-input> <device-conformance-input>" >&2
  exit 2
fi
...
NORMAL_AUTO_RECLAIM="${GENESIS_GPU_GFX_HEADROOM_NORMAL_AUTO_RECLAIM:-0}"
...
if [[ "$REQUIRE_DEVICE_LANE" == "1" && ! -f "$DEVICE_CONFORMANCE_REPORT" ]]; then
  echo "gpu-gfx-headroom: required device conformance report is missing: $DEVICE_CONFORMANCE_REPORT" >&2
  echo "gpu-gfx-headroom: produce it first with: bash scripts/update_gpu_compute_device_conformance_report.sh" >&2
  exit 1
fi
...
report_input = pathlib.Path(sys.argv[19])
failures = [x for x in sys.argv[20:] if x]
...
if report_input.is_file():
    try:
        prev = json.loads(report_input.read_text(encoding="utf-8"))
