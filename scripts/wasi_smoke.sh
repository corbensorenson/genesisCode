...
NATIVE_BIN="${GENESIS_WASI_SMOKE_NATIVE_BIN:-}"
if [[ -z "$NATIVE_BIN" ]]; then
  genesis_configure_cargo_target_dir \
    "$ROOT_DIR" \
    "wasi-smoke-native" \
    root-host
  cargo build -p gc_cli --bin genesis --locked >/dev/null
  NATIVE_BIN="$TMP_DIR/genesis-native"
  cp "$CARGO_TARGET_DIR/debug/genesis" "$NATIVE_BIN"
  chmod 0755 "$NATIVE_BIN"
fi
if [[ "$NATIVE_BIN" != /* || ! -x "$NATIVE_BIN" ]]; then
  echo "GENESIS_WASI_SMOKE_NATIVE_BIN must be an absolute executable path" >&2
  exit 1
fi
NATIVE=("$NATIVE_BIN" --selfhost-artifact "$SELFHOST_ARTIFACT")
...
if [[ -z "${WASM_BIN}" ]]; then
  ...
  cargo build -p gc_wasi_cli --target wasm32-wasip1 --release --locked >/dev/null
...
NATIVE_HASH="$("${NATIVE[@]}" vcs hash --in tests/spec/coreform/map_order.in.gc | tr -d '\n')"
...
m::x
GC
NATIVE_EVAL="$("${NATIVE[@]}" eval "$TMP_DIR/eval.gc" | tr -d '\n')"
...
NATIVE_RUN="$("${NATIVE[@]}" run "$TMP_DIR/run.gc" --caps "$TMP_DIR/caps.toml" --log "$TMP_DIR/native.gclog" | tr -d '\n')"
...
NATIVE_REPLAY="$("${NATIVE[@]}" replay "$TMP_DIR/run.gc" --log "$TMP_DIR/native.gclog" | tr -d '\n')"
...
POLICY_H_NATIVE="$("${NATIVE[@]}" store --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-store-put.gclog" put --in "$TMP_DIR/policy.gc" | tr -d '\n')"
...
NATIVE_POLICY_TERM="$("${NATIVE[@]}" store --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-store-get.gclog" get "$POLICY_H_NATIVE" | tr -d '\n')"
...
GC
EVIDENCE_H_NATIVE="$("${NATIVE[@]}" store --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-evidence-put.gclog" put --in "$TMP_DIR/evidence.gc" | tr -d '\n')"
...
GC
COMMIT_H_NATIVE="$("${NATIVE[@]}" store --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-commit-put.gclog" put --in "$TMP_DIR/commit.gc" | tr -d '\n')"
...
NATIVE_GET="$("${NATIVE[@]}" refs --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-refs-get.gclog" get "refs/heads/dev" | tr -d '\n')"
...
set +e
"${NATIVE[@]}" refs --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-refs-cas.gclog" set "refs/heads/dev" "$COMMIT_H_NATIVE" --policy "$POLICY_H_NATIVE" --expected-old nil >/dev/null 2>&1
N_CODE=$?
...
"${NATIVE[@]}" refs --caps "$TMP_DIR/caps_store_refs.toml" --log "$TMP_DIR/native-refs-del.gclog" delete "refs/heads/dev" --policy "$POLICY_H_NATIVE" >/dev/null
...
SNAP_H_NATIVE="$("${NATIVE[@]}" store --caps "$TMP_DIR/caps_pkg.toml" --log "$TMP_DIR/native-snap-put.gclog" put --in "$TMP_DIR/snap.gc" | tr -d '\n')"
...
N_INIT="$("${NATIVE[@]}" pkg --caps "$TMP_DIR/caps_pkg.toml" --log "$TMP_DIR/native-pkg-init.gclog" init --workspace "wasi-smoke" --lock genesis.lock | tr -d '\n')"
...
N_ADD="$("${NATIVE[@]}" pkg --caps "$TMP_DIR/caps_pkg.toml" --log "$TMP_DIR/native-pkg-add.gclog" add "$SPEC" --lock genesis.lock --update-policy manual | tr -d '\n')"
...
N_LOCK="$("${NATIVE[@]}" pkg --caps "$TMP_DIR/caps_pkg.toml" --log "$TMP_DIR/native-pkg-lock.gclog" lock --lock genesis.lock | tr -d '\n')"
...
N_INSTALL="$("${NATIVE[@]}" pkg --caps "$TMP_DIR/caps_pkg.toml" --log "$TMP_DIR/native-pkg-install.gclog" install --lock genesis.lock --frozen | tr -d '\n')"
...
N_PACK="$("${NATIVE[@]}" pack --pkg "$TMP_DIR/pkg_basic/package.toml" | tr -d '\n')"
...
N_TEST="$("${NATIVE[@]}" test --pkg "$TMP_DIR/pkg_basic/package.toml" | tr -d '\n')"
