...
for path, label in (
    ...
    if not path.is_file():
        raise SystemExit(f"render-selfhost-toolchain-review: missing {label} file: {path}")

fresh = json.loads(freshness.read_text(encoding="utf-8"))
if fresh.get("kind") != "genesis/selfhost-freshness-v0.1":
    raise SystemExit(
        "render-selfhost-toolchain-review: unsupported freshness kind in selfhost/toolchain.freshness.json"
    )
...
if not module_paths:
    raise SystemExit(
        "render-selfhost-toolchain-review: no module paths discovered in selfhost/toolchain_manifest.gc"
    )
...
for rel in module_paths:
    ...
    if not path.is_file():
        raise SystemExit(
            f"render-selfhost-toolchain-review: manifest path missing from workspace: {rel}"
        )
...
out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
print(
    "render-selfhost-toolchain-review: wrote "
