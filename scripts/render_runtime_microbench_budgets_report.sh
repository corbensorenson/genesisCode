...
if [[ "$#" -ne 6 ]]; then
  echo "usage: $0 <metrics-output> <slo-output> <runtime-report-output> <runtime-history-output> <metrics-input> <runtime-history-input>" >&2
  exit 2
fi
...
if [[ -z "$MICROBENCH_FEATURES" && "$GPU_BACKEND_POLICY" == "require-device" && \
      -z "${GENESIS_GPU_COMPUTE_DEVICE_RUNTIME_CMD:-${GENESIS_GPU_COMPUTE_DEVICE_BRIDGE_CMD:-}}" ]]; then
...
if [[ "$SKIP_RUN" == "1" ]]; then
  if [[ ! -f "$METRICS_INPUT" ]]; then
    echo "runtime-microbench: GENESIS_RUNTIME_MICROBENCH_SKIP_RUN=1 requires existing report: $METRICS_INPUT" >&2
    exit 2
  fi
...
echo "runtime-microbench: metrics report=$OUT"
