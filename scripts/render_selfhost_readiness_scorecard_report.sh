...
if [[ "$#" -ne 4 ]]; then
  echo "usage: $0 <report-output> <history-output> <previous-report-input> <history-input>" >&2
  exit 2
fi

REPORT_PATH="$1"
HISTORY_PATH="$2"
PREVIOUS_REPORT_PATH="$3"
HISTORY_INPUT_PATH="$4"
...
python3 - "$ROOT_DIR" "$REPORT_PATH" "$HISTORY_PATH" "$PREVIOUS_REPORT_PATH" "$HISTORY_INPUT_PATH" "$BUDGET_MS" "$P95_MIN_SAMPLES" "$GENESIS_BIN" "$DASHBOARD_JSON" "$START_NS" "$STRICT_MODE" "$CRITICAL_TTL_SEC" <<'PY'
...
previous_report_path = pathlib.Path(sys.argv[4])
history_input_path = pathlib.Path(sys.argv[5])
budget_ms = int(sys.argv[6])
p95_min_samples = int(sys.argv[7])
genesis_bin = pathlib.Path(sys.argv[8])
dashboard_path = pathlib.Path(sys.argv[9])
...
def dim_deprecated_bootstrap_refs() -> dict[str, Any]:
    archive_dir = "_".join(["old", "bootstrap"])
    pattern = re.compile(rf"{archive_dir}/rust_semantics|legacy_program_builders")
    count = 0
    files: list[str] = []
    for path in iter_rust_source_files():
        rel = path.relative_to(root).as_posix()
...
def read_critical_report(
    ...
    if missing_before or stale_before:
        ...
        freshness["update_command"] = producer_cmd
...
def dim_critical_gate_truth() -> dict[str, Any]:
    ...
    critical_specs = [
        (
            ...
            "GENESIS_AGENT_GAUNTLET_PROFILE=release-full GENESIS_AGENT_GAUNTLET_REQUIRE_GPU_DEVICE_BACKEND=1 GENESIS_AGENT_GAUNTLET_REPORT=.genesis/perf/agent_capability_gauntlet_release_confidence_report.json GENESIS_AGENT_GAUNTLET_HISTORY=.genesis/perf/agent_capability_gauntlet_release_confidence_history.jsonl bash scripts/update_agent_reference_workflows_report.sh",
        ),
        (
            ...
            "bash scripts/update_agent_workflow_runtime_parity_report.sh",
        ),
        (
            ...
            "GENESIS_AGENT_GENERATIVE_PRIMARY_REPORT=.genesis/perf/agent_capability_gauntlet_native_report.json GENESIS_AGENT_GENERATIVE_SECONDARY_REPORT=.genesis/perf/agent_capability_gauntlet_wasi_report.json GENESIS_AGENT_GENERATIVE_REQUIRE_SECONDARY=1 bash scripts/update_agent_generative_workloads_report.sh",
        ),
        (
            ...
            "bash scripts/update_production_cli_help_surface_report.sh",
        ),
        (
            ...
            "bash scripts/update_gpu_gfx_headroom_conformance_report.sh",
        ),
        (
            ...
            "bash scripts/update_domain_starter_registry_bootstrap_report.sh",
        ),
        (
            ...
            "bash scripts/update_gcpm_target_runtime_pipelines_report.sh",
        ),
    ]
    ...
    if not report_path.is_file():
        return (
            False,
            {
                ...
                "ok": False,
                "missing": True,
                "update_command": producer_cmd or None,
            },
            f"{label}:missing",
        )
...
def dim_runtime_quality_truth() -> dict[str, Any]:
    ...
    specs = [
        (
            ...
            "bash scripts/update_runtime_microbench_budgets_report.sh",
        ),
        (
            "hot_path_runtime",
            ...
            "hot-path-runtime",
            "bash scripts/update_hot_path_budgets_report.sh",
        ),
        (
            ...
            "bash scripts/update_task_concurrency_stress_report.sh",
        ),
        (
    ...
    for key, report_rel, expected_kind, label, producer_cmd in specs:
        checks.append(label)
        ok, detail, error = read_static_report(
            report_rel, expected_kind, label, producer_cmd
        )
        reports[key] = detail
        if not ok and error is not None:
            errors.append(error)
...
if previous_report_path.is_file():
    try:
        prev = json.loads(previous_report_path.read_text(encoding="utf-8"))
