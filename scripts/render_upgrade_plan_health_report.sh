...
if [[ "$#" -lt 7 ]]; then
  echo "usage: $0 <profile-report> <profile-history> <agent-inner-loop-history> <prepush-history> <release-full-history> <warmup-report> <disk-preflight-report> [--profile <dev-fast|agent-inner-loop|prepush-standard|release-full|full-selfhost-cutover>]" >&2
  exit 2
fi

HEALTH_PROFILE_REPORT="$1"
HEALTH_PROFILE_HISTORY="$2"
AGENT_INNER_LOOP_HISTORY="$3"
PREPUSH_HISTORY="$4"
RELEASE_FULL_HISTORY="$5"
HEALTH_WARMUP_REPORT="$6"
HEALTH_DISK_PREFLIGHT_REPORT="$7"
shift 7

PLAN_FILE="${GENESIS_HEALTH_PLAN_FILE:-upgrade_plan.md}"
[[ -f "$PLAN_FILE" ]] || {
  echo "upgrade-plan-health: missing plan file: $PLAN_FILE" >&2
  exit 2
}
...
PREPUSH_REQUIRE_MIN_HISTORY="${GENESIS_HEALTH_PREPUSH_REQUIRE_MIN_HISTORY:-1}"
PREPUSH_HISTORY_SCOPE_KEY="${GENESIS_HEALTH_PREPUSH_HISTORY_SCOPE_KEY:-prepush-standard-v1}"
RELEASE_FULL_WALL_BUDGET_MS="${GENESIS_HEALTH_RELEASE_FULL_BUDGET_MS:-2700000}"
RELEASE_REFERENCE_CEILING_MS=2700000
RELEASE_ARTIFACT_BUDGET_BYTES=21474836480
RELEASE_FULL_BASELINE_HISTORY="${GENESIS_HEALTH_RELEASE_FULL_BASELINE_HISTORY:-policies/perf/upgrade_plan_health_release_full_seed_history.jsonl}"
RELEASE_FULL_MIN_HISTORY="${GENESIS_HEALTH_RELEASE_FULL_MIN_HISTORY:-3}"
RELEASE_FULL_REQUIRE_MIN_HISTORY="${GENESIS_HEALTH_RELEASE_FULL_REQUIRE_MIN_HISTORY:-1}"
...
HEALTH_AUTO_RECLAIM="${GENESIS_HEALTH_AUTO_RECLAIM:-0}"
...
HEALTH_AUTO_AGGRESSIVE_RECLAIM_ON_LOW_DISK="${GENESIS_HEALTH_AUTO_AGGRESSIVE_RECLAIM_ON_LOW_DISK:-0}"
...
print(int(s.f_bavail) * int(s.f_frsize))
PY
}

generated_artifact_bytes() {
  local total_kib=0 path value
  for path in .genesis/build target node_modules; do
    [[ -e "$path" ]] || continue
    value="$(du -sk "$path" | awk '{print $1}')"
    total_kib=$((total_kib + value))
  done
  echo "$((total_kib * 1024))"
}
...
EOF
    ...
    *"bash scripts/check_ai_stress_suite.sh"*)
      cat <<'EOF'
ai-stress-suite
scripts/check_ai_stress_suite.sh
scripts/render_ai_stress_suite_report.sh
crates/gc_effects/src/**/*.rs
crates/gc_effects/tests/gfx_gpu_bridge.rs
crates/gc_effects/tests/host_abi_surface.rs
crates/gc_cli/tests/cli_selfhost_gpu_parallel.rs
crates/gc_kernel/src/**/*.rs
crates/gc_prelude/src/**/*.rs
selfhost/**/*.gc
EOF
...
scripts/check_domain_starter_registry_bootstrap.sh
...
enforce_prepush_history_budget() {
  ...
  local -a args=(
    ...
    --extra-json "{\"configured_shards\":$HEALTH_SHARDS,\"profile_shards\":$PROFILE_SHARDS,\"cargo_gate_shards\":$HEALTH_CARGO_GATE_SHARDS,\"gate_count\":$gate_count,\"profile_non_cargo_gate_count\":$profile_non_cargo_gate_count,\"profile_cargo_gate_count\":$profile_cargo_gate_count,\"profile_gate_cache_enabled\":$profile_gate_cache_enabled,\"warm_cargo_cache_enabled\":$warm_cargo_cache_enabled,\"wall_budget_ms\":$PREPUSH_WALL_BUDGET_MS,\"generated_disk_delta_bytes\":$profile_generated_disk_delta_bytes,\"generated_disk_budget_bytes\":$PREPUSH_DISK_BUDGET_BYTES,\"artifact_bytes\":$profile_artifact_bytes}"
  )
...
CARGO_GATE_ENTRYPOINTS="$(python3 - "$ROOT_DIR/genesis.gates.json" <<'PY'
import json, pathlib, sys
doc = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
print("\n".join(gate["entrypoint"] for gate in doc["gates"] if gate["compilation"]))
PY
)"
...
command_uses_cargo() {
  ...
  if [[ "$cmd" == *" bash "* || "$cmd" == bash\ * ]]; then
    ...
    if [[ -f "$script_path" ]]; then
      if printf '%s\n' "$CARGO_GATE_ENTRYPOINTS" | grep -Fxq "$script_path"; then
        return 0
      fi
...
run_health_proactive_reclaim() {
  if [[ "$HEALTH_AUTO_RECLAIM" == "1" || "$HEALTH_AUTO_AGGRESSIVE_RECLAIM_ON_LOW_DISK" == "1" ]]; then
    echo "upgrade-plan-health: checks cannot reclaim build caches; dry-run and review scripts/reclaim_build_space.sh --profile dev-clean, execute the confirmed plan, then set reclaim controls to 0" >&2
    return 2
  fi
  echo "upgrade-plan-health: proactive reclaim disabled (read-only check boundary)"
}
...
usage() {
  cat <<'EOF'
Usage: scripts/render_upgrade_plan_health_report.sh <profile-report> <profile-history> <agent-inner-loop-history> <prepush-history> <release-full-history> <warmup-report> <disk-preflight-report> [--profile <dev-fast|agent-inner-loop|prepush-standard|release-full|full-selfhost-cutover>]
EOF
}
...
if (( RELEASE_FULL_WALL_BUDGET_MS > RELEASE_REFERENCE_CEILING_MS )); then
  echo "upgrade-plan-health: release-full budget cannot exceed GB-4 reference ceiling (${RELEASE_REFERENCE_CEILING_MS}ms)" >&2
  exit 2
fi
...
if health_profile_is_strict "$PROFILE" && (( FREE_KB_AFTER_RECLAIM < HEALTH_STRICT_RUNTIME_MIN_FREE_KB )); then
  ...
  echo "upgrade-plan-health: remediation: free disk manually or dry-run, review, and execute a confirmed dev-clean plan with scripts/reclaim_build_space.sh, then rerun the check." >&2
  exit 1
else
  HEALTH_DISK_PREFLIGHT_REASON="ok"
fi
...
if [[ "$declared_open" -gt 0 ]]; then
  ...
  if [[ "$ENFORCE_GATES" != "1" && "$PROFILE" == "dev-fast" ]]; then
    ...
    MANDATORY_LOCAL_NON_CARGO_GATES=(
      ...
      "bash scripts/check_capability_evidence_ledger_contract.sh"
...
case "$PROFILE" in
  ...
  release-full)
    ...
    PROFILE_GATES+=("GENESIS_HEALTH_EVIDENCE_REQUIRED=1 GENESIS_HEALTH_EVIDENCE_MANIFEST='$HEALTH_EVIDENCE_ROOT/manifest.json' GENESIS_WRITE_SKILL_CONFORMANCE_PROFILE=release-full GENESIS_WRITE_SKILL_DIST_VERIFY_RUNTIME=1 GENESIS_WRITE_SKILL_GAUNTLET_REPORT='$HEALTH_EVIDENCE_ROOT/agent_capability_gauntlet_report.json' GENESIS_WRITE_SKILL_GENERATIVE_REPORT='$HEALTH_EVIDENCE_ROOT/agent_generative_workloads_report.json' GENESIS_WRITE_SKILL_RUNTIME_BACKEND_REPORT='$HEALTH_EVIDENCE_ROOT/runtime_backend_feature_matrix_report.json' GENESIS_WRITE_SKILL_HOST_BRIDGE_REPORT='$HEALTH_EVIDENCE_ROOT/host_bridge_fault_injection_report.json' GENESIS_WRITE_SKILL_GPU_XR_REPORT='$HEALTH_EVIDENCE_ROOT/gpu_xr_productization_kits_report.json' GENESIS_WRITE_SKILL_ASSURANCE_REPORT='$HEALTH_EVIDENCE_ROOT/assurance_profile_packs_report.json' bash scripts/check_write_genesiscode_skill_distribution.sh")
    ...
    PROFILE_GATES+=(
      "GENESIS_TASK_STRESS_RUNS=3 GENESIS_TASK_STRESS_ITERATIONS=8 GENESIS_TASK_STRESS_MAX_FAILURE_RATE_PCT=0 GENESIS_TASK_STRESS_SUITE_BUDGET_MS=240000 bash scripts/check_task_concurrency_stress.sh"
    )
    PROFILE_GATES+=(
...
profile_generated_disk_delta_bytes=$((profile_start_free_bytes > profile_end_free_bytes ? profile_start_free_bytes - profile_end_free_bytes : 0))
profile_artifact_bytes="$(generated_artifact_bytes)"
gate_count=$(( \
...
if [[ "$PROFILE" == "dev-fast" ]]; then
  ...
  if (( profile_generated_disk_delta_bytes > PREPUSH_DISK_BUDGET_BYTES )); then
    profile_ok=0
  fi
  ...
  if (( profile_artifact_bytes > RELEASE_ARTIFACT_BUDGET_BYTES )); then
    profile_ok=0
  fi
fi
...
if (( profile_ok == 0 )); then
  if [[ "$PROFILE" == "dev-fast" ]]; then
    ...
    if (( profile_generated_disk_delta_bytes > PREPUSH_DISK_BUDGET_BYTES )); then
      echo "upgrade-plan-health: prepush generated-disk delta exceeded budget (${profile_generated_disk_delta_bytes}B > ${PREPUSH_DISK_BUDGET_BYTES}B)" >&2
    fi
  elif [[ "$PROFILE" == "release-full" ]]; then
    if (( elapsed_ms > RELEASE_FULL_WALL_BUDGET_MS )); then
    ...
    if (( profile_artifact_bytes > RELEASE_ARTIFACT_BUDGET_BYTES )); then
      echo "upgrade-plan-health: release-full artifact footprint exceeded budget (${profile_artifact_bytes}B > ${RELEASE_ARTIFACT_BUDGET_BYTES}B); run the deterministic dev-clean flow first" >&2
    fi
  else
