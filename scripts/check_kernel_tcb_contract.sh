...
REPORT_FILE="${GENESIS_KERNEL_TCB_REPORT:-.genesis/perf/kernel_tcb_contract_report.json}"
...
print(
    ...
    f"(surface_files={len(observed_files)} max_lines={max_lines} report={report_path.relative_to(root).as_posix()})"
)
PY
