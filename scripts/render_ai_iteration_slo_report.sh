...
require_robust_median_sample_count() {
  ...
  if (( value < 3 || value % 2 == 0 )); then
    fail "$name must be an odd integer of at least 3 (got '$value')"
  fi
}
...
require_robust_median_sample_count "GENESIS_AI_ITERATION_SLO_SAMPLES_INCREMENTAL_WARM" "$SAMPLES_INCREMENTAL_WARM"
require_robust_median_sample_count "GENESIS_AI_ITERATION_SLO_SAMPLES_CHANGED_FAST" "$SAMPLES_CHANGED_FAST"
require_robust_median_sample_count "GENESIS_AI_ITERATION_SLO_SAMPLES_CORE_SUITE" "$SAMPLES_CORE_SUITE"
require_robust_median_sample_count "GENESIS_AI_ITERATION_SLO_SAMPLES_GCPM_LOCK" "$SAMPLES_GCPM_LOCK"
require_robust_median_sample_count "GENESIS_AI_ITERATION_SLO_SAMPLES_GCPM_ENV" "$SAMPLES_GCPM_ENV"
...
run_core_suite() {
  GENESIS_SELFHOST_TOOLCHAIN_ARTIFACT="$TOOLCHAIN" \
...
if [[ "$HISTORY_INPUT" != "$HISTORY_OUT" ]]; then
  if [[ -f "$HISTORY_INPUT" ]]; then
    cp "$HISTORY_INPUT" "$HISTORY_OUT"
  else
    : >"$HISTORY_OUT"
  fi
fi
...
def matches_history_scope(row):
    return all(row.get(key) == value for key, value in history_scope.items())

baseline_candidates = baseline_history + history
history_for_baseline = [row for row in baseline_candidates if matches_history_scope(row)]
...
report = {
    **entry,
    ...
    "baseline_candidate_rows": len(baseline_candidates),
    "baseline_scope_rows": len(history_for_baseline),
