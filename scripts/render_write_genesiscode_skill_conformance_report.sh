...
if [[ "$#" -ne 10 ]]; then
  echo "usage: $0 <report-output> <history-output> <history-input> <manifest-input> <gauntlet-input> <generative-input> <runtime-backend-input> <host-bridge-input> <gpu-xr-input> <assurance-input>" >&2
  exit 2
fi
...
require_evidence() {
  ...
  local producer="$3"
  [[ -f "$path" ]] && return 0
  echo "write-genesiscode-skill-conformance: missing $label input: $path" >&2
  echo "write-genesiscode-skill-conformance: produce it with: $producer" >&2
  exit 1
}
require_evidence \
  "agent-gauntlet" \
  "$GAUNTLET_REPORT" \
  "GENESIS_AGENT_GAUNTLET_PROFILE=$PROFILE bash scripts/update_agent_reference_workflows_report.sh"
require_evidence \
  "generative-workload" \
  "$GENERATIVE_REPORT" \
  "GENESIS_AGENT_PARITY_GAUNTLET_PROFILE=$PROFILE bash scripts/update_agent_workflow_runtime_parity_report.sh && GENESIS_AGENT_GENERATIVE_PRIMARY_REPORT=.genesis/perf/agent_capability_gauntlet_native_report.json GENESIS_AGENT_GENERATIVE_SECONDARY_REPORT=.genesis/perf/agent_capability_gauntlet_wasi_report.json GENESIS_AGENT_GENERATIVE_REQUIRE_SECONDARY=1 bash scripts/update_agent_generative_workloads_report.sh"
require_evidence \
  "runtime-backend" \
  "$RUNTIME_BACKEND_REPORT" \
  "bash scripts/update_runtime_backend_feature_matrix_report.sh"
require_evidence \
  "host-bridge fault" \
  "$HOST_BRIDGE_REPORT" \
  "bash scripts/update_host_bridge_fault_injection_report.sh"
require_evidence \
  "GPU/XR" \
  "$GPU_XR_REPORT" \
  "GENESIS_GPU_XR_PRODUCTIZATION_AUTO_RUN_GAUNTLET=1 bash scripts/update_gpu_xr_productization_kits_report.sh"
require_evidence \
  "assurance-profile" \
  "$ASSURANCE_REPORT" \
...
history_input_path = pathlib.Path(history_input_path_s)
...
def evidence_ref(path: pathlib.Path, label: str) -> dict:
...
manifest_ref = evidence_ref(manifest_path, "write-genesiscode-skill-manifest")
gauntlet_ref = evidence_ref(gauntlet_path, "agent-capability-gauntlet")
generative_ref = evidence_ref(generative_path, "agent-generative-workloads")
runtime_backend_ref = evidence_ref(runtime_backend_path, "runtime-backend-feature-matrix")
host_bridge_ref = evidence_ref(host_bridge_path, "host-bridge-fault-injection")
gpu_xr_ref = evidence_ref(gpu_xr_path, "gpu-xr-productization-kits")
assurance_ref = evidence_ref(assurance_path, "assurance-profile-packs")
...
if gauntlet.get("profile") != profile:
    raise SystemExit(
        "write-genesiscode-skill-conformance: gauntlet profile mismatch: "
        f"expected {profile!r}, got {gauntlet.get('profile')!r}"
    )
...
def check_failure_recovery() -> dict:
    ...
    return {
        ...
        "ok": ok,
        "report": host_bridge_ref["identity"],
...
def check_performance_triage() -> dict:
    ...
    return {
        ...
        "ok": ok,
        "report": runtime_backend_ref["identity"],
...
if history_input_path.is_file():
    for raw in history_input_path.read_text(encoding="utf-8").splitlines():
        ...
        if (
            isinstance(row, dict)
            and row.get("kind") == "genesis/write-genesiscode-skill-conformance-v0.1"
            and row.get("profile") == profile
            and isinstance(row.get("score"), int)
            and int(row.get("min_score", -1)) == min_score
        ):
...
report = {
    ...
    "profile": profile,
    "runtime_profile": gauntlet.get("runtime_profile"),
    "manifest_path": manifest_ref["identity"],
    "required_domains": required_domains_ordered,
    "gauntlet_report": gauntlet_ref["identity"],
    "generative_report": generative_ref["identity"],
    "runtime_backend_report": runtime_backend_ref["identity"],
    "host_bridge_report": host_bridge_ref["identity"],
    "gpu_xr_report": gpu_xr_ref["identity"],
    "assurance_report": assurance_ref["identity"],
    "input_evidence": {
        "manifest": manifest_ref,
        "gauntlet": gauntlet_ref,
        "generative": generative_ref,
        "runtime_backend": runtime_backend_ref,
        "host_bridge": host_bridge_ref,
        "gpu_xr": gpu_xr_ref,
    ...
    "rubric": rubric,
    "history_samples": len(prior_history) + 1,
    "timestamp_utc": dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(),
}
if prior_history:
    previous_score = int(prior_history[-1]["score"])
    report["previous_score"] = previous_score
    report["score_delta"] = score - previous_score
