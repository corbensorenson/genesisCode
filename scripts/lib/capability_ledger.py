...
DEFAULT_SEMANTIC_OWNERSHIP_LEDGER = (
    ROOT / "docs/spec/SEMANTIC_OWNERSHIP_LEDGER_v0.1.json"
)
DEFAULT_SEMANTIC_OWNERSHIP_SCHEMA = (
    ROOT / "docs/spec/SEMANTIC_OWNERSHIP_LEDGER_v0.1.schema.json"
)
DEFAULT_SEMANTIC_OWNERSHIP_SPEC = (
    ROOT / "docs/spec/SEMANTIC_OWNERSHIP_LEDGER_v0.1.md"
)
DEFAULT_SELFHOST_CLOSURE_CONTRACT = (
    ROOT / "docs/spec/SELFHOST_CLOSURE_LEVELS_v0.1.json"
)
...
def load_semantic_ownership_ledger() -> Mapping[str, Any]:
    try:
        ledger = semantic_ledger_contract.load_json(DEFAULT_SEMANTIC_OWNERSHIP_LEDGER)
        schema = semantic_ledger_contract.load_json(DEFAULT_SEMANTIC_OWNERSHIP_SCHEMA)
        closure = semantic_ledger_contract.load_json(DEFAULT_SELFHOST_CLOSURE_CONTRACT)
        semantic_ledger_contract.validate(
            ROOT,
            ledger,
            schema,
            DEFAULT_SEMANTIC_OWNERSHIP_SPEC.read_bytes(),
            DEFAULT_SEMANTIC_OWNERSHIP_SCHEMA.read_bytes(),
            closure,
        )
        return ledger
    except (OSError, ValueError) as exc:
        raise LedgerError(f"invalid semantic ownership authority: {exc}") from exc
...
def render_selfhost_status(semantic_ledger: Mapping[str, Any]) -> str:
    decisions = semantic_ledger["semanticDecisions"]
    applicable = [row for row in decisions if row["applicability"] == "applicable"]
    residual = [row for row in decisions if row["applicability"] == "residual-stage0"]
    ...
    for row in applicable:
        ...
        if level is None:
            ...
            counts[level] += 1
    aggregate = "below H0" if below_h0 else min(
        (row["currentLevel"] for row in applicable),
        key=lambda level: SELFHOST_LEVELS.index(level),
    )
    lines = [
        ...
        "",
        f"Audit date: `{semantic_ledger['auditDate']}`",
        f"Ledger identity: `{semantic_ledger['contentIdentitySha256']}`",
        "",
        ...
        "## Summary",
        "",
        f"- Aggregate applicable-decision level: **{aggregate}**",
        f"- Applicable semantic decisions: `{len(applicable)}`",
        f"- Declared residual stage0 decisions: `{len(residual)}`",
        "",
        "| Level | Decisions |",
        "|---|---|",
        f"| Below H0 | {below_h0} |",
    ...
    for row in decisions:
        authorities = "<br>".join(f"`{path}`" for path in row["productionAuthorityPaths"])
        producers = "<br>".join(f"`{path}`" for path in row["producingImplementationPaths"])
        commands = "<br>".join(f"`{item}`" for item in row["commandSelectors"]) or "internal"
        migrations = "<br>".join(f"`{item}`" for item in row["migrationTasks"])
        level = row["currentLevel"] or "below H0"
        lines.append(
            f"| `{row['id']}` | {row['title']} | {row['applicability']} | **{level}** | "
            f"{authorities} | {producers} | `{row['fallbackReachability']}` | "
            f"{commands} | {migrations} |"
        )
    lines.extend(
        [
            "",
...
def expected_outputs(
    ledger: Mapping[str, Any], semantic_ledger: Mapping[str, Any]
) -> Mapping[Path, str]:
    return {
        ...
        DEFAULT_SELFHOST_STATUS: render_selfhost_status(semantic_ledger),
...
def configured_outputs(
    ...
    rendered = expected_outputs(ledger, semantic_ledger)
    return {
...
def main(argv: Sequence[str]) -> int:
    ...
    try:
        ...
        semantic_ledger = load_semantic_ownership_ledger()
        outputs = configured_outputs(ledger, semantic_ledger)
        if args.check:
