...
def validate_sources(root: Path, profile, overrides=None) -> None:
    ...
    for marker in (
        ...
        "decode_result", "result field set mismatch", "request-h",
    ...
    if resolution.count("identity_authority.as_deref_mut()") < 4:
        fail("lock/update resolution does not forward identity authority to every producer/check")
    if ("identity_authority.as_deref_mut()" not in install
