...
genesis_configure_cargo_target_dir "$ROOT_DIR" "selfhost-policy-alias-authority" root-host
...
python3 scripts/lib/selfhost_policy_alias_authority.py \
  ...
  --self-test \
  --runtime \
  --genesis-bin "$CARGO_TARGET_DIR/debug/genesis" \
  --genesis-wasi-bin "$CARGO_TARGET_DIR/debug/genesis_wasi"
