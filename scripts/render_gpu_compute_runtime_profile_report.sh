...
if [[ "$#" -ne 6 ]]; then
  echo "usage: $0 <metrics-output> <guard-output> <runtime-report-output> <runtime-history-output> <metrics-input> <runtime-history-input>" >&2
  exit 2
fi
...
if [[ "$SKIP_RUN" == "1" ]]; then
  if [[ ! -f "$METRICS_INPUT" ]]; then
    echo "gpu-compute-runtime-profile: GENESIS_GPU_COMPUTE_RUNTIME_PROFILE_SKIP_RUN=1 requires existing report: $METRICS_INPUT" >&2
    exit 2
  fi
