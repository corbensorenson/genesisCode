...
with tempfile.TemporaryDirectory() as tmp:
    ...
    for target in targets:
        ...
        build_a = run(build_cmd, cwd=workspace)
        build_b = run(build_cmd, cwd=workspace)
        ...
        target_report["bundle_root"] = str(bundle_root)
        ...
        if runtime_cmd:
            ...
            (target_artifact_dir / "runtime_command.txt").write_text(
                f"synthetic-adapter:{launch_sh} --boot/--smoke\n",
                encoding="utf-8",
            )
            (target_artifact_dir / "runtime_stdout.log").write_text(
                synthetic_log,
        ...
        runtime_evidence = {
            ...
            "command": runtime_cmd if runtime_cmd else f"{launch_sh} --boot/--smoke",
            ...
            "replay_artifact_dir": str(target_artifact_dir),
        }
