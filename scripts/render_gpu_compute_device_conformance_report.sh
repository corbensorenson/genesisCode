...
if [[ "$#" -ne 4 ]]; then
  echo "usage: $0 <artifact-output-dir> <summary-output> <microbench-history-input> <compute-history-input>" >&2
  exit 2
fi

OUT_DIR="$1"
REPORT_OUT="$2"
MICROBENCH_HISTORY_INPUT="$3"
COMPUTE_HISTORY_INPUT="$4"
...
MICROBENCH_RUNTIME_REPORT_OUT="$OUT_DIR/runtime_microbench_runtime_report.json"
MICROBENCH_RUNTIME_HISTORY_OUT="$OUT_DIR/runtime_microbench_runtime_history.jsonl"
COMPUTE_RUNTIME_REPORT_OUT="$OUT_DIR/gpu_compute_runtime_profile_runtime_report.json"
COMPUTE_RUNTIME_HISTORY_OUT="$OUT_DIR/gpu_compute_runtime_profile_runtime_history.jsonl"
...
GENESIS_RUNTIME_MICROBENCH_FEATURES="$MICROBENCH_FEATURES" \
  ...
  bash scripts/render_runtime_microbench_budgets_report.sh \
    "$RUNTIME_METRICS_OUT" \
    "$SLO_OUT" \
    "$MICROBENCH_RUNTIME_REPORT_OUT" \
    "$MICROBENCH_RUNTIME_HISTORY_OUT" \
    "$RUNTIME_METRICS_OUT" \
    "$MICROBENCH_HISTORY_INPUT"
...
GENESIS_GPU_COMPUTE_RUNTIME_PROFILE_REQUIRED_BACKEND="device-runtime" \
  ...
  bash scripts/render_gpu_compute_runtime_profile_report.sh \
    "$PROFILE_OUT" \
    "$PROFILE_GUARD_OUT" \
    "$COMPUTE_RUNTIME_REPORT_OUT" \
    "$COMPUTE_RUNTIME_HISTORY_OUT" \
    "$PROFILE_OUT" \
    "$COMPUTE_HISTORY_INPUT"

python3 - "$RUNTIME_METRICS_OUT" "$SLO_OUT" "$PROFILE_OUT" "$PROFILE_GUARD_OUT" "$OUT_DIR" "$REPORT_OUT" "$LANE_ID" "$GPU_VENDOR" "$OS_FAMILY" "$MICROBENCH_RUNTIME_REPORT_OUT" "$MICROBENCH_RUNTIME_HISTORY_OUT" "$COMPUTE_RUNTIME_REPORT_OUT" "$COMPUTE_RUNTIME_HISTORY_OUT" <<'PY'
import json
import pathlib
import platform
import re
...
microbench_runtime_report_path = pathlib.Path(sys.argv[10])
microbench_runtime_history_path = pathlib.Path(sys.argv[11])
compute_runtime_report_path = pathlib.Path(sys.argv[12])
compute_runtime_history_path = pathlib.Path(sys.argv[13])
...
summary = {
    ...
    "timing_artifacts": {
        "runtime_microbench_timing_report": str(microbench_runtime_report_path),
        "runtime_microbench_timing_history": str(microbench_runtime_history_path),
        "gpu_compute_timing_report": str(compute_runtime_report_path),
        "gpu_compute_timing_history": str(compute_runtime_history_path),
    },
}
