...
GAUNTLET_REPORT="${GENESIS_GPU_XR_PRODUCTIZATION_GAUNTLET_REPORT:-.genesis/perf/agent_capability_gauntlet_report.json}"
WEBXR_REPORT="${GENESIS_GPU_XR_PRODUCTIZATION_WEBXR_REPORT:-.genesis/perf/webxr_browser_conformance_report.json}"
REPORT_OUT="${GENESIS_GPU_XR_PRODUCTIZATION_REPORT:-.genesis/perf/gpu_xr_productization_kits_report.json}"
REQUIRE_WEBXR_RUNTIME_EVIDENCE="${GENESIS_GPU_XR_REQUIRE_WEBXR_RUNTIME_EVIDENCE:-0}"
...
if [[ ! -f "$GAUNTLET_REPORT" && "$AUTO_RUN_GAUNTLET" == "1" ]]; then
  GENESIS_AGENT_GAUNTLET_PROFILE=prepush-standard bash scripts/check_agent_reference_workflows.sh >/dev/null
fi
...
spec_path = root / sys.argv[3]
gauntlet_path = root / sys.argv[4]
webxr_path = root / sys.argv[5]
...
report = {
    ...
    "gauntlet_report": gauntlet_path.relative_to(root).as_posix(),
    "webxr_report": webxr_path.relative_to(root).as_posix(),
