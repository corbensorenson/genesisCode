...
DISK_AUTO_RECLAIM="${GENESIS_AGENT_REFERENCE_WORKFLOWS_DISK_AUTO_RECLAIM:-0}"
...
retained_history_path = pathlib.Path(sys.argv[5])
baseline_history_path = pathlib.Path(sys.argv[6])
default_max_ms = int(sys.argv[7])
require_min_history = sys.argv[8].strip().lower() not in {"0", "false", "no", "off"}
regression_percent = float(sys.argv[9])
...
history_paths = [baseline_history_path]
for candidate in (retained_history_path, history_path):
    if candidate not in history_paths:
        history_paths.append(candidate)
history_durations = load_workflow_duration_history(history_paths, runtime_profile)
...
elapsed_history_samples = load_elapsed_history(history_paths, runtime_profile)
...
report = {
    ...
    "retained_history_path": str(retained_history_path),
    "require_min_history": require_min_history,
    "regression_percent": regression_percent,
    "regression_slack_ms": regression_slack_ms,
