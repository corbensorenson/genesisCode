...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_browser_runtime_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-browser-runtime-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *"browser-first-party-runtime"* || "$run_out" != *"browser-host"* ]]; then
  echo "agent-browser-runtime-workflow: expected browser runtime evidence fields, got=$run_out" >&2
  exit 1
fi

echo "agent-browser-runtime-workflow: ok replay=$run_out"
