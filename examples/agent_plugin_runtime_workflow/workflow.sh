...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_plugin_runtime_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-plugin-runtime-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":ok true"* || "$run_out" != *"host-ok"* || "$run_out" != *"editor-ok"* ]]; then
  echo "agent-plugin-runtime-workflow: expected ok=true with host/editor statuses, got=$run_out" >&2
  exit 1
fi

echo "agent-plugin-runtime-workflow: ok replay=$run_out"
