...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_interactive_gfx_compute_workflow"
...
if [[ ! "$acceptance_h" =~ ^[0-9a-f]{64}$ ]]; then
  echo "agent-interactive-gfx-compute-workflow: expected acceptance hash, got: $acceptance_h" >&2
  exit 1
fi
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-interactive-gfx-compute-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi
...
vcs_h="$("$GENESIS_BIN" \
  --selfhost-only \
  --selfhost-artifact "$ART" \
  vcs hash --in "$WORK_DIR/interactive.gc" --engine selfhost \
  | tr -d '\n')"
if [[ ! "$vcs_h" =~ ^[0-9a-f]{64}$ ]]; then
  echo "agent-interactive-gfx-compute-workflow: expected VCS hash, got: $vcs_h" >&2
  exit 1
fi

echo "agent-interactive-gfx-compute-workflow: ok acceptance=$acceptance_h vcs=$vcs_h replay=$run_out"
