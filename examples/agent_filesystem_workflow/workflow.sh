...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_filesystem_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-filesystem-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":ok true"* || "$run_out" != *":exists true"* ]]; then
  echo "agent-filesystem-workflow: expected ok/exists true, got=$run_out" >&2
  exit 1
fi

echo "agent-filesystem-workflow: ok replay=$run_out"
