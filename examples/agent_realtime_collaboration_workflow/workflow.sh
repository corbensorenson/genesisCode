...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_realtime_collaboration_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-realtime-collaboration-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":core/kit/realtime-merge-result.v1"* || "$run_out" != *":ops-applied 3"* ]]; then
  echo "agent-realtime-collaboration-workflow: expected realtime merge result with ops-applied=3, got=$run_out" >&2
  exit 1
fi

echo "agent-realtime-collaboration-workflow: ok replay=$run_out"
