...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_ml_pipeline_variant_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-ml-pipeline-variant-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":core/kit/ml-run-result.v1"* || "$run_out" != *"tabular-fast"* ]]; then
  echo "agent-ml-pipeline-variant-workflow: expected ml run result with variant=tabular-fast, got=$run_out" >&2
  exit 1
fi

echo "agent-ml-pipeline-variant-workflow: ok replay=$run_out"
