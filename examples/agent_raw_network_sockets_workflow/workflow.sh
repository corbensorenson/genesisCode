...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_raw_network_sockets_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-raw-network-sockets-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":ok true"* || "$run_out" != *"tcp-1"* ]]; then
  echo "agent-raw-network-sockets-workflow: expected ok=true and tcp stream id, got=$run_out" >&2
  exit 1
fi

echo "agent-raw-network-sockets-workflow: ok replay=$run_out"
