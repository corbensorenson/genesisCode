...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_backend_topology_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-backend-topology-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":core/kit/backend-plan-result.v1"* || "$run_out" != *":service-nodes 3"* ]]; then
  echo "agent-backend-topology-workflow: expected backend plan result with service-nodes=3, got=$run_out" >&2
  exit 1
fi

echo "agent-backend-topology-workflow: ok replay=$run_out"
