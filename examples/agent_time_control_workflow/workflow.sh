...
EXAMPLE_DIR="$ROOT_DIR/examples/agent_time_control_workflow"
...
if [[ "$run_out" != "$replay_out" ]]; then
  echo "agent-time-control-workflow: run/replay mismatch: run=$run_out replay=$replay_out" >&2
  exit 1
fi

if [[ "$run_out" != *":ok true"* ]]; then
  echo "agent-time-control-workflow: expected ok=true, got=$run_out" >&2
  exit 1
fi

echo "agent-time-control-workflow: ok replay=$run_out"
